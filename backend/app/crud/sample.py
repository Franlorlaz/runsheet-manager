...
from app.utils import generate_citic_id_for_sample
...
def create_sample(*, db: Session, sample_in: SampleCreate, creator_id: uuid.UUID, parent_sample_id: uuid.UUID | None) -> Sample:
    prefix = generate_citic_id_for_sample([], prefix_only=True)
    existing_citic_ids = db.exec(
        select(Sample.citic_id).where(Sample.citic_id.startswith(prefix))
    ).all()
    citic_id = generate_citic_id_for_sample(existing_citic_ids, prefix=prefix)
